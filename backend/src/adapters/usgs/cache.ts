 ...
 * responses. Mirrors the EONET cache: VOLATILE ONLY (never persisted —
 * GeoResponde is a federator, not a store). It shields the USGS fdsnws budget
 * and is bounded with oldest-key eviction to cap memory under key-cardinality
 * flooding. Earthquakes are volatile, so the TTL is short.
 */
export class UsgsCache {
  private readonly store = new Map<string, CacheEntry>();
  private readonly ttlMs: number;
  private readonly maxEntries: number;

  constructor(options: { ttlMs?: number; maxEntries?: number } = {}) {
    this.ttlMs = options.ttlMs ?? 5 * 60 * 1000; // 5 minutes — quakes are volatile
   ...
   * when USGS is unreachable. Undefined only if never set.
   */
